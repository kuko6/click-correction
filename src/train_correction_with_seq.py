...
from data.correction_generator import CorrectionDataLoader, CorrectionMRIDatasetSequences
...
def main():
    ...
    loss_functions = {
        "dice": DiceLoss(),
        "correction": CorrectionLoss(dims=(1, config["cuts"]["size"], config["cuts"]["size"]), device=device, batch_size=config["batch_size"], inverted=False),
        "invertedCorrection": CorrectionLoss(dims=(1, config["cuts"]["size"], config["cuts"]["size"]), device=device, batch_size=config["batch_size"], inverted=True)
    }
